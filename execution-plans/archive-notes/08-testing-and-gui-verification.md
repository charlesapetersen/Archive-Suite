...
## Bounded sub-tasks
Each is one fresh overnight session: own worktree → `xcodegen generate` → clean build, **no new warnings** → the named tests green → GUI check where applicable → docs move in the same commit → push → remove worktree (00-overview §14; root CLAUDE.md "How we work").
