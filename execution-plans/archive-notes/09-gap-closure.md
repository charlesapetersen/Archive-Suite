...
**A6. Fix stale `SUITE_TODO.md` entries. — ✅ DONE (W9 Phase A docs, 2026-07-18 `8851317`).** — W1's "S5 docs" read shipped despite A1–A4; the "ArchiveCore extraction … deferred, 2026-07-08" sub-bullet contradicted W0 `[x]`; the active-plans index omitted this plan and still listed the shipped `local-agent-cli-provider.md` as "READY".
- *Files:* `SUITE_TODO.md`. *Shipped (4 corrections):* (1) corrected the W1 "S5 docs" note — app-local README/AGENTS/SMOKE_TEST shipped later under W9 Phase A `56360f7`, SPEC marker A4 still pending, referencing this plan; (2) removed the stale "ArchiveCore extraction … deferred" sub-bullet (contradicted the W0 `[x]`); (3) added the missing active-plans index entry for `archive-notes/09-gap-closure.md`; (4) marked `local-agent-cli-provider.md` SHIPPED (struck through) in the index — it shipped W13.cli-1…4 and its plan file was deleted. *Verified:* every active index entry now points to a file on disk; every shipped plan is struck through. *Tier-1.*

**A7. Refresh both apps' `CLAUDE.md` Implementation Maps for the ArchiveCore move (W0 doc-sync). — ✅ DONE (W9 Phase A docs, 2026-07-18).** — `ArchiveReader/CLAUDE.md` listed `DocumentTags`/`TagReading`/`TagEditing`/`PDFFormatStatus` under `Core/` ("package-ready → future ArchiveCore") though all four moved to ArchiveCore; `ArchiveProcessor/CLAUDE.md` hotspot list stopped at "the two `project.yml` files." *Shipped:* **Reader map** — removed the 4 moved types from `Core/`, retitled `Core/` "Reader-local", rewrote `TagWriter` as the *delta adapter* over `ArchiveCore.CoordinatedTagWriter`, added a dedicated **`packages/ArchiveCore`** section (shared types + the `CoordinatedTagWriter` write-choke-point safety note + the `swift test` build lane + `project.yml` wiring), and banner-marked the stale "long-term convergence" section ✅ SHIPPED (package + monorepo landed). **Processor map** — added `packages/ArchiveCore` as a shared hotspot (choke-point/adapter safety note + Tier-2 "build+test all three apps" rule + build lane) and annotated `MacOSTagger` as the *fresh-write adapter* (no direct `setResourceValue`). *Verified against the tree:* `ls packages/ArchiveCore/Sources/…` (13 files across Tags/PDF/Links/Thumbnails + marker), `grep import ArchiveCore` (both apps + `project.yml packages: ArchiveCore`), `TagWrite.swift:114 public enum CoordinatedTagWriter`; Reader `Core/` no longer contains the 4 types.
