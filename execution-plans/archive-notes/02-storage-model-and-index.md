...
## Bounded sub-tasks
Each is sized to one fresh autonomous session (own worktree; build clean, no new warnings; commit + push + flip the `SUITE_TODO.md` Archive Notes W2 checkbox for that sub-task + update `execution-plans/archive-notes/02-*.md` progress; remove worktree). "GUI check" uses `./launch.sh notes` (added in W1). Tests run via the Notes `test-smoke.sh` (added in W1; if not, run `xcodebuild test` directly).
