...
run_fix() {
  printf 'fixture-password\n' | \
    PATH="$T/bin:$PATH" SECURITY_LOG="$SECURITY_LOG" AUTONOMOUS_STATE="${3:-$T/state}" \
...
echo "[3] a marker write failure cannot report a successful repair"
BLOCKED_STATE="$T/blocked-state"; : > "$BLOCKED_STATE"
OUT="$(run_fix 'Gemini' '' "$BLOCKED_STATE")"; rc=$?
[ "$rc" != 0 ] && ok "unwritable marker path makes repair fail" || bad "marker write failure returned success: $OUT"
printf '%s\n' "$OUT" | grep -q 'Could not write the durable repair marker' \
  && ok "marker failure is reported clearly" || bad "marker failure message missing: $OUT"
[ ! -e "$BLOCKED_STATE/keychain-partition-fixed" ] && ok "no partial marker was left behind" || bad "partial marker was created"

echo "[4] the daemon helper finds a later-added present provider, not an absent one"
...
echo "[5] per-item UTC modification checks survive the local UTC offset"
