...
## Autonomous daemon — full review, Wave 32 (2026-08-16)
...
- [ ] **`W32.plist-relogin` — every "stop" is only as durable as the login session: the LaunchAgent plist
...
- [ ] **`W32.stop-lock` — `daemon.sh stop` never clears `engine.lock`, so the next `start` no-ops for up to
...
- [ ] **`W32.prove-stale` — the daemon's own regression suite has been RED on main since 2026-08-13: 19
...
- [ ] **`W32.needs-you-blind` — `status-digest.sh`'s "Needs you" is structurally blind to the Daemon Report:
...
- [ ] **`W32.dr-preamble` — the compactor's region trim-target ignores each region's preamble, so a region
...
- [ ] **`W32.fingerprint-primary` — progress, the attempt cap and the health-gate cadence are all measured
...
- [ ] **`W32.restart-counters` — two of the four park counters survive a daemon restart, so a restart can park
...
- [ ] **`W32.dry-run-stop` — `daemon.sh --dry-run stop` performs a REAL stop [S · MED · ops].** The `DRYRUN`
...
- [ ] **`W32.park-reason` — every park is logged as "SIGTERM … the laptop lid closing", so the one diagnostic
...
- [ ] **`W32.tree-cpu` — `_tree_cpu` truncates the running total on every iteration, so a tree of sub-1%
...
- [ ] **`W32.path-growth` — `tick()` prepends the shim dir to the DAEMON's own `PATH` every cycle, unbounded
...
- [ ] **`W32.usage-guess` — the "usage-limit fast-fail" verdict is a 10-second timing guess when the
...
- [ ] **`W32.preflight-gap` — the daemon re-execs without daemon.sh's prerequisite checks, and guards neither
...
- [ ] **`W32.lock-heartbeat-race` — `park_run` deletes the lock while the heartbeat that recreates it is still
...
- [ ] **`W32.pass3-overrun` — the compactor's Pass 3 can archive `[x]` items out of a section it was never
...
- [ ] **`W32.pass3-safe-tags` — Pass 3's "done-state independently recorded" invariant is defeated by ordinary
...
- [ ] **`W32.pass3-tear` — Pass 3 tears a done item whose write-up contains a blank line, leaving orphaned
...
- [ ] **`W32.fence-order` — `check-tracker-sync.sh` ends the WORK QUEUE region on a `## ` inside a code fence,
...
- [ ] **`W32.classify-names` — `_classify_red`'s document-step list names two steps that do not exist
...
- [ ] **`W32.source-guard` — three destructive side effects run ABOVE the source guard whose comment promises
...
- [ ] **`W32.stub-count` — the ticked-stub guard and the counter it protects disagree on what an item is
...
- [ ] **`W32.docfix-note` — the doc-budget park note asserts the compactor ran even when it did not
...
- [ ] **`W32.complete-substring` — the stale-COMPLETE guard is an unanchored substring match [XS · LOW · ops].**
...
- [ ] **`W32.budget-space` — `_budget_files` takes awk `$3` as the whole path [XS · LOW · ops].** `:544` means a
...
- [ ] **`W32.compact-total` — dead computation in the compactor [XS · LOW · ops].** `compact-plan.sh:151`
...
- [ ] **`W32.label-state` — `$LABEL` does not name the state dir its own comment says it names
