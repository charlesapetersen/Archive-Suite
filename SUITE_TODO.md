...
- [ ] Tag card: when the Spotlight tag index is still building (`SystemTagsProvider.warmUp` not finished) as the operator tries to set tags, present UI saying so (e.g. a "building tag suggestions…" note / disabled-with-hint autocomplete) instead of silently showing no suggestions. | Views/LiveCaptureView.swift (SegmentTagCard), Tagging/SystemTagsProvider.swift | S
- [ ] After rotation review, if finalize/processing is still running, show a throbber / "still processing…" indicator so the gap before collection naming doesn't look hung. | Views/LiveCaptureView.swift, Capture/LiveCaptureProcessor.swift | S  ← now testable unattended via `scripts/android-ui-drive.sh`
