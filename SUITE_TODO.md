...
- [ ] **Headless GUI-test lane for the daemon — Tart macOS VM (LONG-TERM; blocked on disk, 2026-07-18).** macOS has no `Xvfb`: XCUITest and the live sighted loop (`ops/gui/README.md` §2) need the one console `WindowServer`, so on this Mac a real UI test *is* the owner's screen — which is why the daemon runs only `-only-testing:<UnitBundle>` today (`ops/autonomous/README.md` §health gate). A **Tart** VM (`brew install cirruslabs/cli/tart`, Virtualization.framework) gives the guest its own virtual framebuffer, so full XCUITest runs inside it and never touches the host screen; the "Enable UI Automation"/TCC prompt is pre-seeded once (SIP off + `DevToolsSecurity -enable` inside the guest) then snapshotted → prompt-free; the daemon `tart run` (headless) + SSH + `xcodebuild test`s the UITest bundle. **Prereqs already green:** arm64, macOS 26.5.1, Xcode 26.3, `kern.hv_support=1`, Homebrew present. **Blocker:** needs ~120–150 GB free (base image + Xcode + CoW clone + repo/DerivedData in-guest); disk is 97% full (30 GB free) and only ~4–11 GB is dev-cache-reclaimable (biggest: 7.8 GB iOS-26.3 sim runtime, wanted back for the iOS companion) — not enough without deleting real corpus data, and there's no secondary SSD. **Revisit** when internal disk frees up or an external SSD is available (point `TART_HOME` at it). **Cheaper interim (no VM):** keep expanding the permission-free headless render lane (`RenderProbe`/`DocumentRenderGuardTests`/`SnapshotTests`) that already runs in the gate — covers render/layout, just not live E2E interaction. | owner-gated (disk) | L
