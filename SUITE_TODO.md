...
- [ ] Android `targetSdk` 34→36 — **toolchain + bump DONE 2026-07-08** (builds clean); on-device behavior smoke REMAINS (owner/device). Done: installed `platforms;android-36` + `build-tools;36.0.0`; AGP 8.6.1→8.9.1; Gradle 8.9→8.11.1; `compileSdk`/`targetSdk` 34→36; `:app:assembleDebug` **BUILD SUCCESSFUL** on JDK 21. **REMAINING (owner/device, before the ~2026-08-31 Play deadline):** on-device **Android 15/16 behavior smoke** — edge-to-edge enforcement, foreground-service-type rules, runtime-permission changes — on an API-35/36 device or emulator image (the `ap_test` emulator is API-34, so this can't be verified headless here). | files: ArchiveCapture/ (build.gradle.kts, app/build.gradle.kts, gradle wrapper) | S-remaining | med
