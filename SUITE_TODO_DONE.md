...
## Archive Notes — folder graph mutation ordering

- [x] **W9.d2.review-folder-graph — folder mutations can cross folder deletion [M, Tier-2].** **SHIPPED 2026-09-26 (this commit).** `OrganizationStore` now serializes folder creation, rename, move, deletion, and template-assignment changes across each awaited SQLite write and the in-memory/`organization.json` publish. A child or template assignment waiting behind a deletion rechecks its target and refuses the missing folder; move likewise validates its new parent. Deterministic scratch-store regressions pause after the database write, prove the competing operation queued, then exercise rename/delete, move/delete, child-create/delete, and template-assign/delete interleavings. Notes Debug build and full unit smoke passed; independent Tier-2 find→refute review found and closed the template-assignment gap and test synchronization weakness, then found no remaining concrete race. No real Notes store or corpus was touched. | ArchiveNotes/macOS/Sources/ArchiveNotes/Index/OrganizationStore.swift, ArchiveNotes/macOS/Tests/ArchiveNotesTests/OrganizationStoreTests.swift | M | low | done
...
- [x] **W24.jpeg1b — `DurableLink` pins the resolved JPEG path [S-M · Tier-2 · shared ArchiveCore].** **SHIPPED 2026-09-25 (this commit).** `readerReveal` now carries an optional JPEG relative path and serializes it as a required `jpeg` query item; `jpeg=` explicitly records no partner. Parsing rejects pre-JPEG link shapes and malformed page/path fields, while URL query encoding preserves paths that need escaping. Migrated Reader and Notes pattern matches, test writers, fixtures, and retained contract examples. Tier-2 find→refute review found and fixed stale G5 and G6 fixture URLs; the final review found no remaining issue. Verification: ArchiveCore 110 tests; Reader 405 unit tests; Notes 883 Swift Testing checks in 89 suites plus 218 XCTest checks; Processor Debug build and 5 off-screen UI tests; Reader write-surface lint clean; durable-link filesystem E2E 12/12; focused G5 off-screen VM test passed. The focused G6 VM run produced no fresh test log and is not counted as verified. No real corpus, Notes store, or host GUI used.
  | Files: packages/ArchiveCore/Sources/ArchiveCore/Links/DurableLink.swift, packages/ArchiveCore/Tests/ArchiveCoreTests/DurableLinkTests.swift, ArchiveReader/macOS/{Sources/ArchiveReader/Core/DeepLinkRouter.swift,Tests/ArchiveReaderTests/{ArchiveLinkWriterTests,DocumentPageLinkTests}.swift}, ArchiveNotes/macOS/{Sources/ArchiveNotes/{Editor/EditorFormatting.swift,Sources/SourceBlockPaster.swift,Views/{ReaderPreviewPopover,SourceBlockThumbnailRenderer}.swift},Tests/ArchiveNotesTests/{DurableLinkE2ETests,SourceBlockPasterTests}.swift,Tests/ArchiveNotesUITests/NotesGUITests.swift}, ArchiveNotes/scripts/{e2e-durable-links.sh,make-notes-fixture.sh}, execution-plans/{archive-notes/00-overview.md,devonthink-import.md}, SUITE_TODO.md | S-M | med | done

## Archive Notes — gap closure (2026-09-20)

- [x] **W23.l3-fu — Notes had a duplicate, uncoordinated root-marker writer [XS–S · LOW · SHARED CORE].** **SHIPPED 2026-09-26 (this commit).** `RootMarkerStore.ensureMarker` now delegates to `ArchiveCore.RootMarker.ensure`, sharing its coordinated re-check, winner adoption, atomic write and disk confirmation. Notes maps `.malformed`, `.unreadable` and `.readOnly` into its existing `MarkerError.corruptRootMarker` case. Scratch tests verify an empty malformed identity is preserved and eight concurrent Notes first touches return the single durable GUID. Independent Tier-2 find→refute reviews found no confirmed regression. Verification: ArchiveCore 110 tests; Reader 412 tests; Notes 884 Swift Testing checks in 89 suites and 218 XCTest checks; Processor Debug build succeeded. No real Notes store, archive corpus, or host GUI used.
  | Files: ArchiveNotes/macOS/{Sources/ArchiveNotes/Store/RootMarkerStore.swift,Tests/ArchiveNotesTests/RootMarkerStoreTests.swift}, ArchiveNotes/CLAUDE.md, ArchiveReader/KNOWN_ISSUES.md, SUITE_TODO.md | XS–S | low | done
...
- [x] **W9.c6 — 100k-note / 2M-word scale target had no acceptance test [M · Tier-2].** **SHIPPED 2026-09-25 (this commit).** The opt-in scratch-only acceptance suite creates 100,000 UUID-folder Markdown notes with 2,000,000 words inside the test host's writable temporary container. It measures full disk indexing, FTS search, `allSummaries()`, and navigation filtering/sorting. Baseline navigation recompute took 1.713 s on the main actor; corpus-scale filter/sort now runs off-main with generation checks and cooperative cancellation. The final run measured indexing 28.750 s, search <1 ms, summaries 0.272 s, and navigation 1.293 s total, with 0.004 ms scheduling and 0.035 ms main-actor application. Independent Tier-2 review found and closed a detached-task cancellation gap; the stale-filter regression and all 879 Swift Testing tests in 89 suites pass (218 XCTest checks). No real Notes store or corpus used.
  | Files: ArchiveNotes/macOS/{Sources/ArchiveNotes/Core/NotesNavigationModel.swift,Tests/ArchiveNotesTests/{NotesScalePerfTests,NotesNavigationModelTests}.swift,project.yml}, ArchiveNotes/scripts/test-scale-acceptance.sh, ArchiveNotes/KNOWN_ISSUES.md, execution-plans/archive-notes/09-gap-closure.md, SUITE_TODO.md | M | med | done
...
- [x] **W9.b7 — guided root re-grant is not wired [S].** **SHIPPED 2026-09-25 (this commit; implementation in `493cb42`).** The source preview now offers a folder picker that grants the chosen Reader root to Notes and re-resolves the waiting link. The chooser reports a wrong-archive result instead of silently asking again. Existing scratch tests cover correct-root resolution and wrong-root rejection; Notes Debug build and smoke passed (875 Swift Testing tests / 88 suites + 218 XCTest checks).
  | Files in implementation commit: ArchiveNotes/macOS/Sources/ArchiveNotes/Views/ReaderPreviewPopover.swift, Links/ReaderRootChooser.swift, Links/ReaderLinkResolver.swift, Links/ReaderRootStore.swift; ArchiveNotes/macOS/Tests/ArchiveNotesTests/ReaderRootChooserTests.swift | S | low | done
...
- [x] **W3.net-r1 — ✅ SHIPPED 2026-09-26 (this commit).** The shared `CaptureValidation.isSafeGroupId` now
  rejects the empty string, so FileRelay quarantines its JPEG and sidecar without ingesting them; the LAN
  routes retain their explicit empty checks. Tier-2 review found no counterexample. Verification: Processor
  Debug build; FileRelay offline regression 12/12; synthetic-fixture headless phone↔Mac E2E passed all token
  and year assertions. No real corpus or host GUI used. | Net | Tier-2
