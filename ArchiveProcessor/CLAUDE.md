...
- **On the phone:** full-res shutter; **Box** (red) / **Folder** (purple) markers; minimal on-phone tagging (the pre-W19.q7 priority wire field still carries P7–P10 + per-page P10, year/month). In enabled tagging modes, the Mac accepts that P-based *phone input* only at the boundary and writes canonical `Q1`–`Q3` (or no token for P7), never P, to Finder; **No tagging** leaves it and all metadata untouched, while Copy source otherwise preserves an existing source tag spelling verbatim. A **queue-depth heartbeat** (`POST /phone/status`, `X-Pending`) tells the Mac how many photos are still un-sent.
