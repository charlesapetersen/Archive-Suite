#!/bin/bash
# launch.sh — Launch the Archive Notes macOS app. (Mirrors Archive Reader's launch.sh.)
#
...
APPDIR="macOS"
APP="$APPDIR/build/DD/Build/Products/Debug/ArchiveNotes.app"
EXE="$APP/Contents/MacOS/ArchiveNotes"
BUILD_LOG="/tmp/an-launch-build.log"
...
# executable) — so "launch" always shows the current build. Otherwise foreground the running one.
pid=$(pgrep -x ArchiveNotes | head -1)
relaunch=$need_build
...
if [ -n "${pid:-}" ] && [ "$relaunch" = 0 ]; then
  osascript -e 'tell application "System Events" to set frontmost of (first process whose name is "ArchiveNotes") to true' 2>/dev/null
  echo "✓ Already running the current build — brought to the front."
else
  [ -n "${pid:-}" ] && { pkill -x ArchiveNotes 2>/dev/null; sleep 0.6; }
  open "$APP"
  echo "✓ Launched: $APP"
fi
